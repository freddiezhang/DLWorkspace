        ...
        with BatchPackage.queue_lock:
            for batching_record in record_list:
                if batching_record.bond_failed == True:
                    ...
                    aria.log_manager.LogManager.stats_manager.records_bond_failed(batching_record.tenant, 1)
                    
                    if StatsConstants.stats_tenant_token != batching_record.tenant:
                        aria.log_manager.LogManager.decrement_events_in_memory(1)
                        
                    aria.log_manager.LogManager.subscribers.update(batching_record.tenant, [batching_record.sequence_id], SubscribeStatus.BOND_FAILED)
                    aria.log_manager.LogManager.stats_manager.events_dropped(batching_record.tenant, 1)
                else:
                    try:
                        if aria.log_manager.LogManager.configuration.MAX_SIZE_ALLOWED < batching_record.size:
                            AriaLog.aria_log.debug("Record reached the maximum size allowed")
                            AriaLog.aria_log.debug("Max size=" + str(aria.log_manager.LogManager.configuration.MAX_SIZE_ALLOWED) + ", record_size=" + str(batching_record.size))
                            
                            if StatsConstants.stats_tenant_token != batching_record.tenant:
                                aria.log_manager.LogManager.decrement_events_in_memory(1)
                            
                            aria.log_manager.LogManager.stats_manager.events_dropped(batching_record.tenant, 1)
                            aria.log_manager.LogManager.stats_manager.records_dropped_status(batching_record.tenant, 1, "event_to_big")
                            aria.log_manager.LogManager.subscribers.update(batching_record.tenant, [batching_record.sequence_id], SubscribeStatus.EVENT_TO_BIG)
                            continue
                        
                        found_tenant = False
                        for index in range(len(BatchPackage.queue)):
                            if batching_record.tenant == BatchPackage.queue[index].tenant:
                                if BatchPackage.queue[index].size + batching_record.size < aria.log_manager.LogManager.configuration.MAX_SIZE_ALLOWED:
                                    found_tenant = True
                        ...
                        if aria.log_manager.LogManager.stats_manager != None:
                            if StatsConstants.stats_tenant_token != batching_record.tenant:
                                aria.log_manager.LogManager.decrement_events_in_memory(1)
                            
                            aria.log_manager.LogManager.subscribers.update(batching_record.tenant, [batching_record.sequence_id], SubscribeStatus.PATCHING_FAILED)
                            aria.log_manager.LogManager.stats_manager.records_dropped_status(batching_record.tenant, 1, "packaging_failed")
        ...
        with BatchPackage.queue_lock:
            ...
            for index in range(len(BatchPackage.queue)):
                with Batcher.out_queue_lock:
                    if len(Batcher.out_queue) < aria.log_manager.LogManager.configuration.TCP_CONNECTIONS or Batcher.is_flushed:
