...
class Batcher(object):
    ...
    def flush():
        ...
        Batcher.sender_thread.join(1)
        Batcher.sender_thread_lazy.join(1)
    ...
    def send_all_remaining_events():
        ...
        try:
            ...
            if records != None:
                if aria.log_manager.LogManager.configuration.PROCESS_NUMBER > 0:
                    aria.log_manager.LogManager.pool_process.apply_async(SerializeEvents, args=(records,), callback = batch_package.BatchPackage.add_records).get()
                else:
    ...
    def start_batcher_thread():
        ...
        if aria.log_manager.LogManager.configuration.PROCESS_NUMBER > 0:
            AriaLog.aria_log.debug("Bond serialize process number=" + str(aria.log_manager.LogManager.configuration.PROCESS_NUMBER))
            for i in range(aria.log_manager.LogManager.configuration.PROCESS_NUMBER):
                th = Thread(target = Batcher.__run_batcher)
                th.daemon = aria.log_manager.LogManager.configuration.ALL_THREADS_DEAMON
                th.start()
                Batcher.bathcer_thread_list.append(th)
                
            batcher_th = Thread(target = Batcher.__run_batcher_lazy)
            batcher_th.daemon = aria.log_manager.LogManager.configuration.ALL_THREADS_DEAMON 
            batcher_th.start()
    ...
    def start_sender_thread():
        ...
        Batcher.sender_thread.daemon = aria.log_manager.LogManager.configuration.ALL_THREADS_DEAMON 
        Batcher.sender_thread.start()

    @staticmethod
    def __run_batcher_thread():
        while Batcher.batcher_thread_running == True:
            records = Batcher.remove_records_from_in(aria.log_manager.LogManager.configuration.MAX_EVENTS_TO_BATCH)
            if records != None:
                AriaLog.aria_log.debug("Processing records" + " Records=" + str(len(records)))
                batch_package.BatchPackage.add_records(SerializeEvents(records))
            sleep(aria.log_manager.LogManager.configuration.BATCHER_TIMER)
    ...
    def __run_batcher_lazy_thread():
        while Batcher.batcher_lazy_thread_running == True:
            records = Batcher.remove_record_from_in_leazy(aria.log_manager.LogManager.configuration.MAX_EVENTS_TO_BATCH)
            if records != None:
                AriaLog.aria_log.debug("Processing records" + " Records=" + str(len(records)))
                batch_package.BatchPackage.add_records(SerializeEvents(records))
            sleep(aria.log_manager.LogManager.configuration.LAZY_BATCHER_TIMER)
    ...
    def __run_sender():
        ...
        while Batcher.sender_thread_running == True:
            ...
            if package != None:
                ...
                sleep(aria.log_manager.LogManager.configuration.SENDER_TIMER)
    ...
    def __run_sender_lazy():
        ...
        while Batcher.sender_thread_lazy_running == True:
            ...
            sleep(aria.log_manager.LogManager.configuration.SENDER_LAZY_TIMER)
    ...
    def add_record(tenant, record):        
        ...
        if StatsConstants.stats_tenant_token != tenant:
            aria.log_manager.LogManager.increment_events_in_memory(1)
    ...
    def drop_events():
        ...
        records_to_remove = aria.log_manager.LogManager.configuration.QUEUE_DROPPED_EVENTS
        ...
        while records_to_remove != 0:
            with  Batcher.in_queue_lock:
                if len(Batcher.in_queue) >= records_to_remove:
                    ...
                    for rec in records:
                        aria.log_manager.LogManager.subscribers.update(rec.tenant, [rec.sequence_id], SubscribeStatus.MAX_SIZE_REACHED)
                        aria.log_manager.LogManager.stats_manager.events_dropped(rec.tenant, 1)
                        aria.log_manager.LogManager.stats_manager.records_dropped_status(rec.tenant, 1, "queue_max_size_reached")
                    
                        if StatsConstants.stats_tenant_token != rec.tenant:
                            aria.log_manager.LogManager.decrement_events_in_memory(1)
                ...
                with batch_package.BatchPackage.queue_lock:
                    ...
                    for index in range(len(batch_package.BatchPackage.queue)):
                        if len(batch_package.BatchPackage.queue[index].records_list) >= records_to_remove:
                            aria.log_manager.LogManager.subscribers.update(batch_package.BatchPackage.queue[index].tenant, batch_package.BatchPackage.queue[index].records_list, SubscribeStatus.MAX_SIZE_REACHED)
                            aria.log_manager.LogManager.stats_manager.events_dropped(batch_package.BatchPackage.queue[index].tenant, batch_package.BatchPackage.queue[index].records)
                            aria.log_manager.LogManager.stats_manager.records_dropped_status(batch_package.BatchPackage.queue[index].tenant, batch_package.BatchPackage.queue[index].records, "queue_max_size_reached")
                            if StatsConstants.stats_tenant_token != batch_package.BatchPackage.queue[index].tenant:
                                aria.log_manager.LogManager.decrement_events_in_memory(batch_package.BatchPackage.queue[index].records)
                ...
                with Batcher.out_queue_lock:
                    ...
                    for index in range(len(Batcher.out_queue)):
                        if len(Batcher.out_queue[index].records_list) >= records_to_remove:
                            aria.log_manager.LogManager.subscribers.update(Batcher.out_queue[index].tenant, Batcher.out_queue[index].records_list, SubscribeStatus.MAX_SIZE_REACHED)
                            aria.log_manager.LogManager.stats_manager.events_dropped(Batcher.out_queue[index].tenant, Batcher.out_queue[index].records)
                            aria.log_manager.LogManager.stats_manager.records_dropped_status(Batcher.out_queue[index].tenant, Batcher.out_queue[index].records, "queue_max_size_reached")
                            if StatsConstants.stats_tenant_token != Batcher.out_queue[index].tenant:
                                aria.log_manager.LogManager.decrement_events_in_memory(Batcher.out_queue[index].records)
