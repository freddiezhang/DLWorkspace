...
class AriaSender(object):
    def __init__(self, sender_index):
        ...
        try:
            ...
            self.run_thread.daemon = aria.log_manager.LogManager.configuration.ALL_THREADS_DEAMON
            self.package_to_send = None
            self.package_retry = 0  # TODO implement retry logic
            self.package_size = 0
            self.retry_count = aria.log_manager.LogManager.configuration.MAX_RETRY_COUNT
            ...
            try:
                ...
                if self.package_to_send != None:
                    ...
                    if StatsConstants.stats_tenant_token != self.package_to_send.tenant: 
                        aria.log_manager.LogManager.decrement_events_in_memory(self.package_to_send.records)
                    
                    if response == None: 
                        aria.log_manager.LogManager.subscribers.update(self.package_to_send.tenant, self.package_to_send.records_list, SubscribeStatus.NO_INTERNET_CONNECTION)
                        aria.log_manager.LogManager.stats_manager.events_dropped(self.package_to_send.tenant, self.package_to_send.records)
                        aria.log_manager.LogManager.stats_manager.average_package_size(self.package_to_send.tenant, len(self.package_to_send.serialized))
                        aria.log_manager.LogManager.stats_manager.average_record_size(self.package_to_send.tenant, len(self.package_to_send.serialized)/self.package_to_send.records)
                        continue
                    # Alert all the subscribers  
                    aria.log_manager.LogManager.subscribers.update(self.package_to_send.tenant, self.package_to_send.records_list, response.status)
                        
                    if response.status == 200:
                        aria.log_manager.LogManager.stats_manager.events_send(self.package_to_send.tenant, self.package_to_send.records)
                        aria.log_manager.LogManager.stats_manager.total_data_send(self.package_to_send.tenant, len(self.package_to_send.serialized))
                    else:
                        aria.log_manager.LogManager.stats_manager.events_dropped(self.package_to_send.tenant, self.package_to_send.records)
                        aria.log_manager.LogManager.stats_manager.records_dropped_status(self.package_to_send.tenant, self.package_to_send.records, response.status)
                    ...
                    sleep(aria.log_manager.LogManager.configuration.ARIA_SENDER_TIMER)
            except:
                AriaLog.aria_log.warning("AriaSender has failed" + str(exc_info()[0]))
                sleep(aria.log_manager.LogManager.configuration.ARIA_SENDER_TIMER)
            if aria.log_manager.LogManager.flushed_called == True:
                AriaLog.aria_log.info("Sender aria.LogManager.flushed_called")
                self.run_thread_running = False
                return
    ...
    def send_to_pipeline(self):
        ...
        try:
            while send_tries < self.retry_count:
                ...
                try:
                    if aria.log_manager.LogManager.configuration.SUPPORT_GZIP == True:
                    ...
                    aria.log_manager.LogManager.stats_manager.events_tried(self.package_to_send.tenant, self.package_to_send.records)
                    ...
                    try:
                        ...
                        aria.log_manager.LogManager.stats_manager.records_retry_status(self.package_to_send.tenant, self.package_to_send.records, AriaSender.create_event_name(str(exc_info()[0])))
                    ...
                    if response != None and response.status == 200:
                        ...
                        if response != None:
                            ...
                            aria.log_manager.LogManager.stats_manager.records_retry_status(self.package_to_send.tenant, self.package_to_send.records, response.status)
                        elif retry_cause_found == False:
                            aria.log_manager.LogManager.stats_manager.records_retry_status(self.package_to_send.tenant, self.package_to_send.records, response_error)
                            
                        aria.log_manager.LogManager.stats_manager.records_retry(self.package_to_send.tenant, self.package_to_send.records)
                        send_tries += 1
