...
class StatsConstants(object):
    SDK_VERSION      = "AST_Python_Linux_no_1.3.7.0"
...
class StatsManager(object):
    def __init__(self):
        ...
        self.flushed = False
        StatsConstants.cadence = aria.log_manager.LogManager.configuration.STATS_CADENCE

    def __init_worker_thread(self):
        self.worker_thread = threading.Thread(target = self.__run)
        self.worker_thread.daemon = aria.log_manager.LogManager.configuration.ALL_THREADS_DEAMON 
    ...
    def __sentStats(self, stats):
        ...
        for stat in stats:
            if self.__hasStats(self.stats[stat]) and stat != StatsConstants.stats_tenant_token:
                ...
                LogManager.get_logger("", StatsConstants.stats_tenant_token).log_event(event)
